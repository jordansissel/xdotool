...
the window stack. Generally you will only want the first one as the order of
the windows in the stack is unreliable (See L<XQueryTree> for ordering details).
If you specify '%@' then all windows in the stack will be used.
